...
public class Player: NSManagedObject {
    convenience init(playerName: String, playerId: String, userImageData: Data, context: NSManagedObjectContext){
    ...
    convenience init(playerName: String, playerId: String, context: NSManagedObjectContext){
        if let ent = NSEntityDescription.entity(forEntityName: "Player", in: context){
            self.init(entity: ent, insertInto: context)
            self.playerId = playerId
            self.name = playerName
